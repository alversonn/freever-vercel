...
export default function RecordDetailPage() {
  ...
  // useEffect untuk mengecek status darurat secara dinamis saat mengedit
  useEffect(() => {
    if (isEditing) {
  ...
  // useEffect untuk mengambil data awal saat halaman dimuat
  useEffect(() => {
    if (id) {
      const fetchRecord = async () => {
        ...
        try {
          ...
          setRecord(data);
          setFormData(data);
          // Set status darurat awal berdasarkan data dari database
  ...
  const handleRadioChange = (name: keyof PatientRecord, value: string) => {
  ...
  const handleUpdate = async () => {
    ...
    // Jika setelah diedit menjadi darurat, hapus data step 2 dan perbarui diagnosis
    if (isEmergencyCase) {
  ...
  // Komponen Helper untuk menampilkan data atau input field
  const DetailField = ({ label, name, value, type = 'text' }: { label: string; name: keyof PatientRecord; value: any; type?: string }) => (
    <div>
      <Label htmlFor={name} className="text-sm font-semibold">{label}</Label>
      {isEditing ? (
        type === 'textarea' ? (
          <Textarea id={name} name={name} value={formData[name] as string || ''} onChange={handleInputChange} className="mt-1" />
        ) : (
          <Input id={name} name={name} type={type} value={formData[name] as any || ''} onChange={handleInputChange} className="mt-1" />
        )
      ) : (
        <p className="mt-1 p-2 bg-gray-100 rounded-md min-h-[40px]">{value}</p>
      )}
    </div>
  );
        ...
        <CardContent className="space-y-6">
          <section>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <DetailField label="Patient Name" name="name" value={record.name} />
              <DetailField label="Age" name="age" value={record.age} type="number" />
              <DetailField label="Date of Birth" name="dateOfBirth" value={new Date(record.dateOfBirth).toISOString().split('T')[0]} type="date" />
              <div>
              ...
              <div className="md:col-span-2">
                <DetailField label="Address" name="address" value={record.address} type="textarea" />
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid md:grid-cols-3 gap-4 mb-6">
                ...
                <DetailField label="Oxygen Saturation (%)" name="oxygenSaturation" value={record.oxygenSaturation} type="number"/>
            </div>
            ...
            <fieldset disabled={isEditing && isEmergencyCase} className="transition-opacity duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
              ...
              <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-4">
                  <DetailField label="Leukocyte" name="leukocyteCount" value={record.leukocyteCount} type="number"/>
                  <DetailField label="Neutrophil" name="neutrophilCount" value={record.neutrophilCount} type="number"/>
                  <DetailField label="Lymphocyte" name="lymphocyteCount" value={record.lymphocyteCount} type="number"/>
                  <DetailField label="NLCR" name="nlcrResult" value={record.nlcrResult} type="number"/>
                  <DetailField label="CRP Level" name="crpLevel" value={record.crpLevel} type="number"/>
                  <DetailField label="Fever Duration (days)" name="feverDuration" value={record.feverDuration} type="number"/>
              </div>
            </fieldset>
          </section>
